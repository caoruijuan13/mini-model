...
2026-09-18 在当前工作树运行 `python3 -m projects.stage09_subword_tokenizer.compare`。语料为 `data/declaration_excerpt.txt`，沿用 07/08 的顺序 80/10/10 原文切分；读取时移除文件末尾换行。两种 tokenizer 仅在训练文本上建词表，BPE 默认 `max_merges=40`、`min_pair_frequency=2`，实际学习 40 条 merge。统计时逐行编码，不添加 BOS/EOS，保留每行原有换行。
